...
INDEX_PATH = "/root/projects/jules-mcp/src/index.ts"

with open(INDEX_PATH, "r", encoding="utf-8") as f:
    src = f.read()

# 1. Update paths
old_paths = 'const USAGE_PATH = process.env.JULES_USAGE_PATH || path.join(process.env.HOME || "/root", ".config/jules/usage.json");'
new_paths = '''const USAGE_PATH = process.env.JULES_USAGE_PATH || path.join(process.env.HOME || "/root", ".config/jules/usage.json");
...
# 2. Add Queue, Locks, Semantic Resolver Functions
queue_locks_code = '''
...
req_pattern = r'async function request\(endpoint: string, apiKey: string.*?\n\}\n\n// Request with automatic multi-account fallback'
new_req = '''async function request(endpoint: string, apiKey: string, options: https.RequestOptions = {}, body?: any, maxRetries = 3): Promise<any> {
...
src = re.sub(req_pattern, new_req, src, flags=re.DOTALL)
...
src = src.replace('      {\n        name: "jules_pool_status",', new_tool_schemas)
...
# 8. Add acquireFileLock inside jules_recipe_dispatch
src = src.replace(
    'recordDispatch(bestAccount.name || bestAccount.email || "Account", sid, sourceResource);',
    '''recordDispatch(bestAccount.name || bestAccount.email || "Account", sid, sourceResource);
...
print("Upgraded index.ts to v1.6.0 successfully.")
