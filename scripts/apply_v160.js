...
// 1. Paths
const oldPaths = `const USAGE_PATH = process.env.JULES_USAGE_PATH || path.join(process.env.HOME || "/root", ".config/jules/usage.json");`;
const newPaths = `const USAGE_PATH = process.env.JULES_USAGE_PATH || path.join(process.env.HOME || "/root", ".config/jules/usage.json");
...
src = src.replace(oldPaths, newPaths);

// 2. Queue, Locks, Semantic Resolver Functions
const queueLockHelpers = String.raw`
...
// 3. Robust request() with Exponential Backoff + Jitter for Transient Errors
const oldRequestFn = src.slice(
  src.indexOf("async function request("),
  src.indexOf("// Request with automatic multi-account fallback")
);

const newRequestFn = String.raw`async function request(endpoint: string, apiKey: string, options: https.RequestOptions = {}, body?: any, maxRetries = 3): Promise<any> {
  const url = ` + "`${API_BASE}/${endpoint}${endpoint.includes(\"?\") ? \"&\" : \"?\"}key=${apiKey}`;" + String.raw`
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      return await new Promise((resolve, reject) => {
          ...
          res.on("end", () => {
            try {
              ...
              if (res.statusCode && res.statusCode >= 400) {
                const err: any = new Error(` + "`Google Jules API error (${res.statusCode}): ${parsed.error?.message || data}`" + String.raw`);
              ...
              if (res.statusCode && res.statusCode >= 400) {
                const err: any = new Error(` + "`Google Jules API error (${res.statusCode}): ${data}`" + String.raw`);
                err.statusCode = res.statusCode;
                reject(err);
              } else {
                resolve(data);
        ...
        req.on("timeout", () => {
          req.destroy();
          const err: any = new Error(` + "`Request to ${endpoint} timed out.`" + String.raw`);
          err.isTimeout = true;
          reject(err);
        });
        if (body) {
...
src = src.replace(oldRequestFn, newRequestFn);

// 4. Update Version
src = src.replace(`version: "1.5.0",`, `version: "1.6.0",`);

// 5. Add 3 New Tool Schemas
const newToolSchemas = String.raw`      {
...
// 6. Add Tool Handlers
const newToolHandlers = String.raw`    // ----------------------------------------------------
...
// 8. Auto file-lock acquisition in jules_recipe_dispatch
src = src.replace(
  `recordDispatch(bestAccount.name || bestAccount.email || "Account", sid, sourceResource);`,
  `recordDispatch(bestAccount.name || bestAccount.email || "Account", sid, sourceResource);
...
// 9. Semantic Conflict Resolver in jules_consolidate_sessions & jules_apply_patch
const oldConsolidateCatch = String.raw`            } catch (err: any) {
              failedSessions.push({ session_id: cleanSid, reason: ` + "`3-way patch application failed: ${err.message}`" + String.raw` });
            }`;

const newConsolidateCatch = String.raw`            } catch (err: any) {
              // Semantic conflict resolution fallback
              try {
...
src = src.replace(oldConsolidateCatch, newConsolidateCatch);

fs.writeFileSync("/root/projects/jules-mcp/src/index.ts", src, "utf-8");
console.log("Successfully wrote upgraded /root/projects/jules-mcp/src/index.ts (v1.6.0)");
